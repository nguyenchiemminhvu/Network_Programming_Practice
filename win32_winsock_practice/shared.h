...
namespace ClientServer_CompletionRoutineModel
{
    auto f_prompt = [](const std::string& prompt) -> bool { std::cout << prompt; return true; };
...
namespace ClientServer_IOCP_Model
{
    auto f_prompt = [](const std::string& prompt) -> bool { std::cout << prompt; return true; };
    ...
    void TCP_Server()
    {
        ...
        soc_listen_state.operation_type = OP_ACCEPT_COMPLETED;
        ...
        while (true)
        {
            ...
            case OP_ACCEPT_COMPLETED:
                ...
                accept_state_result->operation_type = OP_READ_COMPLETED;
            ...
            case OP_READ_COMPLETED:
                ...
                {
                    listen_state_result->buffer_len = bytes_transferred;
                    wsa_buffer = { listen_state_result->buffer_len, listen_state_result->buffer };
                    listen_state_result->operation_type = OP_WRITE_COMPLETED;
                    rc = WSASend(
                        listen_state_result->socket,
                        &wsa_buffer,
                        1,
                    ...
                    if (rc == SOCKET_ERROR)
                    {
                        if (WSAGetLastError() != WSA_IO_PENDING)
                        {
                            WS_ERROR("WSASend in OP_READ_COMPLETED failed with code:", WSAGetLastError());
                            return;
                        }
                    }
                }
            ...
            case OP_WRITE_COMPLETED:
                ...
                {
                    flags = 0;
                    wsa_buffer = { 1024, listen_state_result->buffer };
                    memset(listen_overlapped_result, 0, sizeof(WSAOVERLAPPED));
                    listen_state_result->operation_type = OP_READ_COMPLETED;
                    rc = WSARecv(
                        listen_state_result->socket,
                        &wsa_buffer,
                        1,
                ...
                if (rc == SOCKET_ERROR)
                {
                    if (WSAGetLastError() != WSA_IO_PENDING)
                    {
                        WS_ERROR("WSARecv in OP_WRITE_COMPLETED failed with code:", WSAGetLastError());
                        return;
                    }
                }
...
namespace ClientServer_IOCP_Mutithread_Model
{
    auto f_prompt = [](const std::string& prompt) -> bool { std::cout << prompt; return true; };
